...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  public DrivetrainSubsystem(HashMap<ModulePosition, SwerveModule> modules, Gyro gyro) {
    ...
    inputs.moduleStates = new SwerveModuleState[swerveModules.size()];
    inputs.desiredStates = new SwerveModuleState[swerveModules.size()];
  ...
  public void periodic() {
    ...
    for(var entry : swerveModules.entrySet()) {
      entry.getValue().log("Drivetrain/" + entry.getKey().name());
      inputs.moduleStates[entry.getKey().ordinal()] = entry.getValue().getState();
      inputs.desiredStates[entry.getKey().ordinal()] = entry.getValue().getDesiredState();
    }

    inputs.gyroAngle = m_gyro.getContinuousYawDeg();

    // Logger.recordOutput("Drivetrain/gyroAngle", m_gyro.getContinuousYawDeg());
