...
public class SwerveModule {
  ...
  public static class SwerveModuleInputs {
    ...
    public double unoffsetEncoderPosition;
  }
  ...
  public SwerveModule(ModulePosition position, int driveMotorChannel, int turningMotorChannel, int turningEncoderChannel, boolean driveMotorReversed, double turningEncoderOffset, double turningEncoderMinVal, double turningEncoderMaxVal) {
    this(position, driveMotorChannel, turningMotorChannel, turningEncoderChannel, driveMotorReversed, true, turningEncoderOffset, turningEncoderMinVal, turningEncoderMaxVal);
  }
   ...
   */
  public SwerveModule(ModulePosition position, int driveMotorChannel, int turningMotorChannel, int turningEncoderChannel, boolean driveMotorReversed, boolean turningMotorReversed, double turningEncoderOffset, double turningEncoderMinVal, double turningEncoderMaxVal) {
  ...
  public SwerveModuleState getState() {
    ...
    return new SwerveModuleState(speedMetersPerSecond, Rotation2d.fromDegrees(getEncoderPosition()));

  }

  public SwerveModuleState getDesiredState() {
    return new SwerveModuleState(inputs.velocityError, Rotation2d.fromDegrees(inputs.setPosition));
  }
  ...
  public SwerveModulePosition getPosition() {
    ...
    // Rotation2d turnAngleRadians = m_turningEncoder.getAbsolutePositionRotation2D();
    Rotation2d turnAngleRadians = Rotation2d.fromDegrees(getEncoderPosition());
  ...
  public void setDesiredState(SwerveModuleState desiredState) {
    ...
    double currentTurnAngle = getEncoderPosition()*(Math.PI/180);


    // Optimize the reference state to avoid spinning further than 90 degrees
    desiredState = SwerveModuleState.optimize(desiredState, new Rotation2d(currentTurnAngle));
    
    inputs.setPosition = desiredState.angle.getDegrees();
    inputs.setVelocity = desiredState.speedMetersPerSecond;
  ...
  public void log(String name) {
    //inputs.actualPosition = m_turningEncoder.getAbsolutePositionDegrees();
    inputs.actualPosition = getEncoderPosition();
    inputs.actualVelocity = m_driveMotor.get() * DriveConstants.kMaxSpeedMetersPerSecond;
    //inputs.unoffsetEncoderPosition = m_turningEncoder.getUnoffsetPositionDegrees();

    inputs.positionError = inputs.setPosition - inputs.actualPosition;
    inputs.velocityError = inputs.setVelocity - inputs.actualVelocity;

    Logger.processInputs(name, inputs);
  }

  private double getEncoderPosition() {
    return m_turningEncoder.getPosition() * POSITION_CONVERSION_FACTOR;
  }

  private void setEncoderPosition(double position) {
    m_turningEncoder.setPosition(position / POSITION_CONVERSION_FACTOR);
  }
}